...
describe WashOut::Param do
  ...
  describe "booleans" do
    ...
    it "should accept 'true' and '1'" do
      map[0].load({:value => true}, :value).should be_truthy
      map[0].load({:value => "1"}, :value).should be_truthy
    end

    it "should accept 'false' and '0'" do
      map[0].load({:value => false}, :value).should be_falsy
      map[0].load({:value => "0"}, :value).should be_falsy
    end
  end
end
